...
public:
    ...
    Matrix<T> multiplicationVinograd(const Matrix<T>& mat) const;
    Matrix<T> multiplicationVinogradOptimised(const Matrix<T>& mat) const;
    ...
    Matrix<T>& operator~() const; // матрица * -1
    T determinant() const; T det() const; // идентичные, просто с разным названием
    Matrix<T>& makeReversed(); // обратная матрица
    Matrix<T> reversed() const; // обратная матрица
    Matrix<T> operator!(); // обратная матрица
